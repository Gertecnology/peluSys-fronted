...
const InformeVentaProducto = ({ }) => {
    ...
    const obtenerProductos = () => {
        ...
        if (user && user.token) {
            ...
            productoApi
                .getProductos("VENTA", inicioFormateado, cierreFormateado, 0, 100)
                .then((datos) => {
    ...
    const handleFiltrar = (fechaInicio, fechaCierre, marcaInput, nombreInput) => {
        const fechaInicioT = obtenerFechaInicio(fechaInicio); // Obtener la hora de inicio
        const fechaCierreT = obtenerFechaCierre(fechaCierre); // Obtener la hora de cierre
            ...
            .then((datos) => {
                ...
                if (marcaInput || nombreInput) {
                    productosFiltrados = productosFiltrados.filter((producto) => {
                        ...
                        return marcaValida && nombreValido;
                    });
                }
    ...
    return (
        <Layout pagina={"Informe Venta"} titulo={"Ventas de Productos"} ruta={ruta.pathname}>
                        ...
                        <div className="w-1/4">
                            <div className="pt-4">
                                <button
                                    ...
                                    onClick={() => handleFiltrar(fechaInicio, fechaCierre, marcaInput, nombreInput)}
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {isBuscar || isFiltro ? (
                                        (paginatedProductoFiltrado?.length === 0 ? (
                                            <tr>
                                                <td colSpan="6" className="px-6 py-4 text-gray-900 text-center font-bold uppercase">No se encontraron ventas en ese rango.</td>
                                            </tr>
                                        ) : (
                                        ...
                                        (paginatedProducto?.length === 0 ? (
                                            <tr>
                                                <td colSpan="6" className="px-6 py-4 text-gray-900 text-center font-bold uppercase">No se han registrado ventas en este día.</td>
                                            </tr>
                                        ) : (
